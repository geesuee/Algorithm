// 배열로 queue 구현 연습
// 코드 출처 : https://freestrokes.tistory.com/83
...
public class ArrayQueue implements Queue {
    ...
    public boolean isFull() {
        ...
        return(rear == this.queueSize-1);  // size에 비해 index는 하나 작기 때문
    }
    ...
    public char dequeue() {
        if(isEmpty()) {
            ...
            System.out.println("Deleting Item : " + queueArr[front + 1]);

            // front 포인터는 삭제할 위치에 있는 상태이므로 다음과 같이 (front + 1) % size 로 설정
            // front + 1로 설정하면 front 포인터가 마지막 요소에 위치했을 경우, ArrayOutOfBoundException 발생
            // 때문에 (front + 1) % size 로 설정
            // ex) 큐의 size 가 5일 때, index 범위는 0~4
